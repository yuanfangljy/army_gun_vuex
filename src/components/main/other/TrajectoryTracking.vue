...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
      ...
      <div class="layui-input-inline">
        开始时间：
        <input type="type"  class="layui-input"   id="startTime" ref="startTime" placeholder="请选择开始时间" >
      </div>
      &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
      <div class="layui-input-inline">
        截止时间：
        <input type="type"  class="layui-input"   id="endTime" ref="endTime" placeholder="请选择截止时间" >
      </div>

			<div class="modal-footer">
				<button type="button" class="btn btn-default" @click="closeTime()">关闭
				</button>
				<button type="button" class="btn btn-primary" @click="submitTime()">
					确认
				</button>
			</div>
		</div><!-- /.modal-content -->
...
export default {
  ...
  methods: {
    getMapTrajectory() {
      ...
      var value = [];//纬度
      var point =[]; // 添加折线运动轨迹
        _this.$axios.get('/gunLocation/readGunTrajectory?appImei='+_this.appImei+"&imeiType="+_this.imeiType
                                                                  +"&startTimes="+_this.startTime
                                                                  +"&endTimes="+_this.endTime
                                                                  ).then(response=>{
  ...
  //    map.addOverlay(new BMap.Polyline(arrPois, {strokeColor: '#111'})); //不画线
      ...
      marker = new BMap.Marker(_this.arrPois[0], {
        icon: new BMap.Icon(_this.imgType,
          new BMap.Size(52, 26),
   ...
   /*  trackThePlayback(){
    ...
    submitTime(){
      let _this=this;
      var sTime=_this.$refs.startTime.value;
      var eTime=_this.$refs.endTime.value;
      if (!sTime && !eTime) {
  ...
  created() {
    ...
    if(this.imeiType==0){
      this.imgType="/static/assets/img/device.png"
    }else{
      this.imgType="/static/assets/img/device.png"
    }
  },
